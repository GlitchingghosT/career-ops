...
const checks = [
  ...
  [apply.includes('work_authorization.status') && apply.includes('work_authorization.needs_sponsorship') && apply.includes('unknown or missing') && apply.includes('confirmation required') && apply.includes('never auto-answer'), 'application mode never auto-answers when either work-authorization field is unknown or missing'],
