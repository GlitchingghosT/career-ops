...
- **location**: Country, timezone, visa status, on-site availability, and structured work authorization (`work_authorization.status`, `work_authorization.needs_sponsorship`, plus legacy `authorized_in`). Unknown or missing values require candidate confirmation; an explicit no-sponsorship posting becomes a hard blocker only after the candidate's status is confirmed.
