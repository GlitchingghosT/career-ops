...
work_auth: "{confirmation_required | sponsors | not_needed | unstated | no_sponsorship}"
discard_reasons:
...
- `work_auth` reflects the Block A work-authorization tier: `confirmation_required` when `work_authorization.status` or `work_authorization.needs_sponsorship` is unknown (score-neutral and never a hard blocker until the candidate confirms); `no_sponsorship` only when confirmed candidate status and the JD explicitly refuses sponsorship for a role outside the candidate's `authorized_in`; `unstated` when the JD is silent; `not_needed` when confirmed authorization covers the role; `sponsors` when the JD explicitly offers it.
...
**Work Auth:** {❓ Confirmation required | ✅ Sponsors | ➖ Not needed | ⚠️ Unstated | ⛔ No sponsorship}
**URL:** {{URL}}
