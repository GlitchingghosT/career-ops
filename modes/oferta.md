...
After the Role Summary table, compare the candidate's work authorization against what the JD says about sponsorship and work eligibility. Read `config/profile.yml` → `work_authorization.status` and `work_authorization.needs_sponsorship` first, then the legacy `location.authorized_in`, `location.needs_sponsorship`, and free-text `location.visa_status` fields. Classify into exactly one tier:

- ❓ **Confirmation required** — `work_authorization.status` or `work_authorization.needs_sponsorship` is `unknown`, or the profile does not contain enough evidence to answer. This tier takes precedence over every JD signal: it is score-neutral and never a hard blocker until the candidate confirms their status.
...
- Never convert `unknown`, `null`, an empty authorization list, or a missing field into `false` / “Not needed.”
...
- **Scoring (aligns with `modes/_profile.md` "Your Location Policy"):** ❓ / ✅ / ➖ / ⚠️ are score-neutral — do **not** apply a location or relocation penalty. Only ⛔ **No sponsorship** after confirmed candidate status for a role the candidate cannot take from an authorized country is a genuine hard blocker: score location low and record it as a `hard_stop`.
